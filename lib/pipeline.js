...
export function saveSettings(cfg, dir) {
  fs.writeFileSync(path.join(dir, 'settings.json'), JSON.stringify({
    ...
    // Frame filenames in this folder start at frameOffset, not 0 -- see
    // DiskSink in lib/frames.js. Recorded here so a second look at this
    // folder (or a support question) does not have to reverse-engineer it
    // from the scene selection.
    frameOffset: cfg.frameOffset || 0,
...
async function renderLocked(cfg, o, dir, { encode, concurrent, reap, wantSweep }) {
  const sink = new DiskSink(dir, cfg.geom.captureW, cfg.geom.captureH, cfg.frameOffset || 0);
  ...
  if (cfg.redo && cfg.redo.length) {
    ...
    // A --redo target that is not on disk yet needs no special handling -- it
    // is already going to render as part of the normal "fill what's missing"
    // pass below -- so only frames actually dropped here are worth reporting.
    const hit = cfg.redo.filter((i) => i < cfg.totalFrames && sink.has(i));
    for (const i of hit) sink.remove(i);
    if (hit.length) {
      // Reported as the numbers on disk (and the ones the user typed), not
      // the internal 0-based index this render happens to use for a scene.
      console.log(c.dim(`\n  Re-rendering ${hit.length} frame(s) on request: ` +
        `${summariseRanges(hit.map((i) => i + off)).slice(0, 120)}`));
    }
  }
  ...
  if (already) {
    console.log(`\n  ${c.g(`${already}/${cfg.totalFrames}`)} frames already on disk` +
      (missing.length && missing.length <= 60
        ? c.dim(`  · to render: ${summariseRanges(missing.map((i) => i + (cfg.frameOffset || 0)))}`)
        : ''));
  }
  ...
  const feeding = (async () => {
    ...
    for (let i = 0; i < cfg.totalFrames; i++) {
      while (!sink.has(i)) {
        if (captureFailed) throw new Error(`capture failed before frame ${i + (cfg.frameOffset || 0)}`);
        if (captureFinished && !sink.has(i)) {
          throw new Error(`frame ${i + (cfg.frameOffset || 0)} never arrived`);
        }
        await sleep(40);
      }
  ...
  try {
    ...
    if (captureFailed && captureFailed.length) {
      const off = cfg.frameOffset || 0;
      console.log(c.dim(`  Missing: ${summariseRanges(captureFailed.map((i) => i + off)).slice(0, 200)}`));
...
async function sweep(cfg, sink) {
  ...
  return sweepDropouts(cfg, sink, cfg.totalFrames, {
    ...
    maxCandidates: cfg.sweepMaxCandidates || 0,
  });
}
...
async function encodeAll(cfg, sink, { reap }) {
  ...
  try {
    for (let i = 0; i < cfg.totalFrames; i++) {
      if (!sink.has(i)) {
        throw new Error(`frame ${i + (cfg.frameOffset || 0)} is missing; use "Fill gaps" first.`);
      }
      await encoder.push(sink.take(i));
