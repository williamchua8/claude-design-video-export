 ...
 *
 * Frame numbers and timestamps are both read against the FULL video's own
 * numbering -- the same numbering the frame files on disk use (see DiskSink in
 * lib/frames.js) -- not against the local 0-based position within whatever
 * scene is currently selected. `offset` is that scene's starting frame number
 * (0 for a whole-timeline render), subtracted here to land on the right local
 * index. Without this, "--redo 48s" while rendering just one scene would look
 * for a frame near the start of THAT scene instead of at t=48s of the video.
 *
 * @returns {number[]} local indices, sorted, de-duplicated, clamped to [0, total)
 */
export function parseFrameSelection(spec, fps, total, offset = 0) {
  const out = new Set();
  const toFrame = (tok) => /s$/i.test(tok)
    ? Math.round(parseFloat(tok) * fps)
    : parseInt(tok, 10);
  const add = (global) => {
    const local = global - offset;
    if (isFinite(local) && local >= 0 && local < total) out.add(local);
  };
  ...
  for (const raw of String(spec || '').split(',')) {
    ...
    if (m) {
      ...
      for (let g = Math.min(a, b); g <= Math.max(a, b); g++) add(g);
      continue;
    }
    add(toFrame(token));
  }
