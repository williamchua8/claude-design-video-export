 ...
 */
export async function probeProject(input, { raster = 'gpu', channel = null, timeout = 180000 } = {}) {
  const { browser, info } = await launchBrowser({ raster, channel });
  try {
    ...
    try {
      await page.waitForFunction(
        ...
        OM_STAGE_SELECTOR, { timeout: 20000 },
      );
    ...
    await ctx.close();
    return { adapter: detected, browserInfo: info, gpu, pageErrors: errors, ...meta };
  } finally {
