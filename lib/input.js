...
export function pickEntryHtml(rootDir) {
  const all = listHtml(rootDir);
  if (!all.length) return null;
  return all
    .map((fp) => ({ fp, score: scoreHtml(fp, rootDir) }))
    .sort((a, b) => b.score - a.score)[0].fp;
}
...
export function resolveInput(inputPath) {
  ...
  if (st.isDirectory()) {
    ...
    return { rootDir: root, entryFile: entry, fromZip: false, cleanup: () => {} };
  }

  if (!/\.html?$/i.test(abs)) {
    throw new Error(`${path.basename(abs)} is not an .html file, a folder, or a .zip.`);
  }
  return { rootDir: path.dirname(abs), entryFile: abs, fromZip: false, cleanup: () => {} };
}
