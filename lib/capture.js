...
export async function prepareWorker(browser, cfg, id) {
  ...
  const w = { id, ctx, page, pageErrors, virtualT: -1, clip: null, stats: null };
...
export async function captureFrame(w, cfg, index, timeSec) {
  ...
  const need = Math.max(1, cfg.stableCapture || 1);
  if (need < 2) return shoot(w, cfg);
...
export async function runCapture(cfg, todo, sink, { label = 'Rendering', onFrame = null } = {}) {
  ...
  if ((cfg.stableCapture || 1) >= 2) {
    if (stats.restabilised) {
      ...
      console.log(c.dim('    Those would have exported with missing content without --stable-capture.'));
    } else {
      console.log(c.dim('  Stable capture: every frame was complete on the first shot ' +
        '(this machine does not need it here).'));
    }
  ...
  } else if (cfg.stableCapture !== 0 && todo.length > 40) {
    // Only worth suggesting on a run long enough for the problem to show up.
    console.log(c.dim('  (if elements come out missing on this machine, --stable-capture ' +
      'catches it during capture)'));
  }
