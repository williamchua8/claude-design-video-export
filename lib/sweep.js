 ...
 *
 * `offset` is the scene's frame-numbering offset (see DiskSink in
 * lib/frames.js) -- a scene folder's files are not named frame_000000.png
 * onward, they are named starting at wherever that scene sits in the full
 * timeline, so ffmpeg has to be told where to start reading.
 */
export function frameSignatures(dir, total, { width = 192, offset = 0 } = {}) {
  const bin = ffmpegBin();
  if (!bin) return null;
  const r = spawnSync(bin, [
    '-hide_banner', '-loglevel', 'error',
    '-start_number', String(offset),
...
export async function sweepDropouts(cfg, sink, total, opts = {}) {
  const dir = sink.dir;
  const t0 = Date.now();
  // A saturated render needs more than two passes: each one only re-renders up
  // to the candidate cap, so a scene with three capfuls of suspects would
  // otherwise finish with two thirds of them untouched.
  const passes = Math.max(1, opts.passes || 2);
  const maxPasses = Math.max(passes, 6);
  const fps = cfg.fps || 30;
  const frameOffset = cfg.frameOffset || 0;
  ...
  for (let pass = 1; pass <= maxPasses; pass++) {
    ...
    const sig = frameSignatures(dir, total, { offset: frameOffset });
    if (!sig) {
    ...
    for (const r of found.slice(0, 8)) {
      // Reported as the frame numbers on disk and the absolute video time, not
      // this render's own 0-based position -- a scene render's frame 0 is not
      // t=0 of the video.
      const g0 = r.start + frameOffset, g1 = r.start + r.len - 1 + frameOffset;
      const atT = (r.start / fps + (cfg.timeOffset || 0)).toFixed(2);
      console.log(c.dim(`    frames ${r.len === 1 ? g0 : `${g0}-${g1}`} ` +
        `(t=${atT}s) — ${r.source === 'held'
    ...
    if (det.capped) {
      sawCapped = true;
      console.log(c.y(`    (${det.capped} lower-scoring candidate(s) held over to the next pass -- ` +
        `raise --sweep-max-candidates to do more at once)`));
    }
    ...
    const careful = {
      ...
      // Always on for a repair, whatever the run asked for: these frames are
      // already known to be suspect, so paying an extra screenshot each to
      // guarantee the compositor finished is trivially worth it -- and it is
      // the difference between a repair that fixes the frame and one that
      // reproduces the same half-drawn picture and gets rejected.
      stableCapture: Math.max(2, cfg.stableCapture || 0),
      stableTries: Math.max(4, cfg.stableTries || 0),
    };
    await runCapture(careful, targets.map((i) => ({ index: i, time: t0off + i / fps })),
      sink, { label: `Re-rendering suspects${pass > 1 ? ` (pass ${pass})` : ''}` });

    const sig2 = frameSignatures(dir, total, { offset: frameOffset });
    ...
    // Stop when nothing improved -- but NOT while candidates are still held
    // over by the cap. A pass where every visible suspect turned out to be
    // authored content would otherwise throw away the ones that never got
    // looked at, which is exactly what happens on a heavily affected render.
    if (!repairedThisPass && !det.capped) break;
    // Past the requested pass count, keep going only to drain the backlog.
    if (pass >= passes && !det.capped) break;
  }
  ...
  if (repairedFrames.length) {
    console.log(`  ${c.g(`Repaired ${repairedFrames.length} frame(s)`)}: ` +
      `${summariseRanges(repairedFrames.map((i) => i + frameOffset)).slice(0, 160)}`);
  ...
  if (keptFrames.length) {
    console.log(c.dim(`  ${keptFrames.length} frame(s) reproduced the same way — ` +
      `authored content, left as-is: ` +
      `${summariseRanges(keptFrames.map((i) => i + frameOffset)).slice(0, 160)}`));
  }
  // A render this heavily affected is being patched up after the fact when it
  // should not be dropping frames in the first place.
  if (sawCapped || allFound.length > 20) {
    console.log(c.y('\n  This render produced an unusual number of suspect frames.'));
    console.log(c.dim('  The sweep repairs them, but --stable-capture prevents them: it re-shoots'));
    console.log(c.dim('  each frame until the picture stops changing, which costs about one extra'));
    console.log(c.dim('  screenshot per frame and is far cheaper than repairing this many.'));
  }
