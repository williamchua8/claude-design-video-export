 ...
 *  'cpu-raster' -- --disable-gpu-rasterization ONLY. Tiles are filled on the
 *             CPU, but the GPU compositor is kept, so preserve-3d depth sorting
 *             still works. This is the middle option for a machine that drops
 *             frames under 'gpu' but needs correct 3D stacking, which 'software'
 *             cannot give it. Reasoned from what each Chromium flag does rather
 *             than measured here: this sandbox has no discrete GPU, so 'gpu' is
 *             already SwiftShader and the driver-specific fault cannot be
 *             reproduced in it. Try it against --stable-capture and keep
 *             whichever is faster for you.
 *  'software' -- --disable-gpu --disable-gpu-compositing. Sometimes steadier on
 *             very layer-heavy scenes, but it costs TWO things, not one:
 *
 *               - the path most likely to box a large blur (the original
 *                 reason this is a fallback, not a default); and
 *               - real 3D depth sorting for `transform-style: preserve-3d` /
 *                 `translateZ` content. Chromium's software compositor was
 *                 confirmed (by reading a real Claude Design composition's
 *                 source, which used both heavily for a "camera" over a stack
 *                 of Z-offset cards) to fall back to PAINT ORDER for stacking
 *                 without the GPU compositor -- so a card that should sit in
 *                 front, per its transform, can render behind one that comes
 *                 later in the DOM instead. That reads as "an element is
 *                 partially hidden behind something it should be in front of,"
 *                 and it is specific to software raster: the same composition
 *                 is correct under 'gpu'.
 *
 *             So --software is a real fallback for the paint-dropout and
 *             layer-pressure cases it was added for, but the wrong choice for
 *             any composition using a 3D card stack or camera -- which is a
 *             common Claude Design pattern, not a rare one. If elements look
 *             right under 'gpu' and wrong under 'software' in a scene with
 *             depth or a stack of panels, that is this, not a new bug: switch
 *             back to 'gpu' and look at --paint-determinism or --jobs 1 for
 *             the dropout problem instead.
 */
function rasterArgs(raster) {
  // Two independent things get called "software", and conflating them is what
  // forces the bad trade above:
  //
  //   GPU RASTERISATION -- who fills in the pixels of each layer's tiles.
  //   GPU COMPOSITING   -- who assembles those layers into a frame, and which
  //                        is what implements real 3D depth sorting for
  //                        preserve-3d / translateZ content.
  //
  // --disable-gpu-compositing is the one that loses depth sorting. Disabling
  // rasterisation alone does not, so 'cpu-raster' keeps 3D correct while still
  // taking the tile-filling work off the GPU driver.
  if (raster === 'software') return ['--disable-gpu', '--disable-gpu-compositing'];
  if (raster === 'cpu-raster') return ['--disable-gpu-rasterization'];
  return ['--ignore-gpu-blocklist', '--enable-gpu-rasterization'];
}

export const RASTER_MODES = ['gpu', 'cpu-raster', 'software'];
